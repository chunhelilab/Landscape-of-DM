function [xd, yd, d, pr, flux_x, flux_y, F_data, flux_x1, flux_y1] = flux_2D(input, t, a)
...
d = floor((ma - mi) / a) + 1;  %the number of squares
pr = zeros(d);
flux_x = zeros(d);
flux_y = zeros(d);
[xd, yd] = meshgrid(mi:a:ma, mi:a:ma);

%% coarse graining to esitmate flux and probability
...
for i = 2:n
    ...
    qy = ky - ky0;
    q = abs(qx) + abs(qy); 
    if q == 1  %two points are adjacent
    ...
    elseif q > 1 %two points are nonadjacent
        if qx == 0 %the second point is on the y-axis direction of the first point
            flux_y(ky0, kx0) = flux_y(ky0, kx0) + 0.5 * sign(qy);
            flux_y(ky, kx) = flux_y(ky, kx) + 0.5 * sign(qy);
            for j = 1:(abs(qy) - 1)
                flux_y(ky0 + j * sign(qy), kx0) = flux_y(ky0 + j * sign(qy), kx0) + sign(qy);
            end
        elseif qy == 0 %the second point is on the x-axis direction of the first point
            flux_x(ky0, kx0) = flux_x(ky0, kx0) + 0.5 * sign(qx);
            flux_x(ky, kx) = flux_x(ky, kx) + 0.5 * sign(qx);
            for j = 1:(abs(qx) - 1)
